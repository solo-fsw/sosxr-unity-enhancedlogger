...
public class DemoEnhancedLoggerTwo : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            // Destroy the GameObject immediately to demonstrate that Log.Static() still works
            // even when the caller object is destroyed. This shows the robustness of the static logging method.
            DestroyImmediate(gameObject);
        }

        Log.Static(
            "This is still shown even though the gameobject will be destroyed.",
            LogLevel.Verbose
        );
        ...
        Log.Static(
            "This is an info message. It is only shown when the log level is set to Info",
            LogLevel.Info
        );
    }
}
